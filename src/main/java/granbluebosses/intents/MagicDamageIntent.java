...
import static granbluebosses.intents.enums.CustomIntentEnums.MAGIC_ATTACK;

public class MagicDamageIntent extends CustomIntent {
     ...
     */
    public MagicDamageIntent() {
        super(MAGIC_ATTACK, TEXT[0],
                GranblueBosses.intentPath("MagicDamageIntent.png"), //These are just the paths to my images.
                GranblueBosses.intentPath("MagicDamageIntentL.png"),  //Will differ for yours, of course.
                "This enemy intends to deal #y${modID}:Magic_Damage.");
    }
    ...
    public String description(AbstractMonster mo) {
        ...
        if (hitNum == -1 || dmgNum == -1){
            result = TEXT[1] + "ERROR" + " #yMagic damage.";
        } else if (hitNum == 1){
            result = TEXT[1] + dmgNum + TEXT[2];
        } else {
            result = TEXT[1] + dmgNum + TEXT[3] + hitNum + TEXT[4];
        }
    ...
    */
    ...
    static {
        ID = makeID(MagicDamageIntent.class.getSimpleName());
