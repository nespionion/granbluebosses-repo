...
import static granbluebosses.GranblueBosses.intentPath;
import static granbluebosses.GranblueBosses.makeID;
import static granbluebosses.intents.enums.CustomIntentEnums.ATTACK_MAGIC;

public class AttackPlusMagicIntent extends CustomIntent {
     ...
     */
    public AttackPlusMagicIntent() {
        super(ATTACK_MAGIC, TEXT[0],
                intentPath("AttackPlusMagicIntent.png"), //These are just the paths to my images.
                intentPath("AttackPlusMagicIntentL.png"),  //Will differ for yours, of course.
                TEXT[5]);
        GranblueBosses.logger.info("Intent Img path: " + intentPath("AttackPlusMagicIntent.png"));
    }
    ...
    public String description(AbstractMonster mo) {
        ...
        int dmgNum = ReflectionHacks.getPrivate(mo, AbstractMonster.class, "intentDmg");


        String result;

        if (hitNum > 1){
            result = TEXT[1] + dmgNum + TEXT[2] + hitNum + TEXT[3] + TEXT[4];
        } else {
            result = TEXT[1] + dmgNum + TEXT[2] + TEXT[4];
        }
    ...
    public String damageNumber(AbstractMonster am) {
        int hitNum = ReflectionHacks.getPrivate(am, AbstractMonster.class, "intentMultiAmt");
        int dmgNum = ReflectionHacks.getPrivate(am, AbstractMonster.class, "intentDmg");

        return hitNum > 1 ? dmgNum + "x" + hitNum : "" + dmgNum;
    }
...
//    }
    ...
    */
    ...
    static {
        ID = makeID(AttackPlusMagicIntent.class.getSimpleName());
