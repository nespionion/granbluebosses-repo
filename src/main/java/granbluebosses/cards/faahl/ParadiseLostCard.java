...
public class ParadiseLostCard extends BaseSignatureCard {

    public static final String ID = makeID(ParadiseLostCard.class.getSimpleName());
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        addToBot(new DamageAllEnemiesAction(p, this.magicNumber, DamageInfo.DamageType.HP_LOSS, AbstractGameAction.AttackEffect.NONE));

        for (AbstractMonster mo : AbstractDungeon.getMonsters().monsters){
            if (!mo.isDying && !mo.isDeadOrEscaped()){
                addToBot(new DispelBuffAction(mo, p, true));
                addToBot(new CancelOmenAction(mo));
            }
        }
