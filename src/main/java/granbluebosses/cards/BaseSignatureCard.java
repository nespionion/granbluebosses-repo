...
import granbluebosses.cards.rewards.Arcarum.ZArcarumEasyUnlock;
...
public abstract class BaseSignatureCard extends AbstractSignatureCard implements OnObtainCard {
    ...
    final protected Map<String, BaseSignatureCard.LocalVarInfo> cardVariables = new HashMap<>();

    public BaseSignatureCard(String ID, CardStats info) {
        this(ID, info, getCardTextureString(removePrefix(ID), info.cardType));
    }
    public BaseSignatureCard(String ID, CardStats info, String cardImage) {
        this(ID, info.baseCost, info.cardType, info.cardTarget, info.cardRarity, info.cardColor, cardImage);
    }
    public BaseSignatureCard(String ID, int cost, CardType cardType, CardTarget target, CardRarity rarity, CardColor color) {
        this(ID, cost, cardType, target, rarity, color, getCardTextureString(removePrefix(ID), cardType));
    }
    public BaseSignatureCard(String ID, int cost, CardType cardType, CardTarget target, CardRarity rarity, CardColor color, String cardImage)
    {
    ...
    public void onObtainCard() {
        ZArcarumEasyUnlock.ids.add(this.cardID);
    }
    ...
    protected final void setCustomVar(String key, int base, int upgrade) {
        ...
        if (!customVars.containsKey(key)) {
            BaseSignatureCard.QuickDynamicVariable var = new BaseSignatureCard.QuickDynamicVariable(key);
    ...
    protected final void setCustomVar(String key, BaseSignatureCard.VariableType type, int base) {
        setCustomVar(key, type, base, 0);
    }
    protected final void setCustomVar(String key, BaseSignatureCard.VariableType type, int base, int upgrade) {
        setCustomVarValue(key, base, upgrade);
    ...
    protected final void setCustomVar(String key, BaseSignatureCard.VariableType type, int base, TriFunction<BaseSignatureCard, AbstractMonster, Integer, Integer> preCalc) {
        setCustomVar(key, type, base, 0, preCalc);
    }
    protected final void setCustomVar(String key, BaseSignatureCard.VariableType type, int base, int upgrade, TriFunction<BaseSignatureCard, AbstractMonster, Integer, Integer> preCalc) {
        setCustomVar(key, type, base, upgrade, preCalc, BaseSignatureCard.LocalVarInfo::noCalc);
    }
    protected final void setCustomVar(String key, BaseSignatureCard.VariableType type, int base, TriFunction<BaseSignatureCard, AbstractMonster, Integer, Integer> preCalc, TriFunction<BaseSignatureCard, AbstractMonster, Integer, Integer> postCalc) {
        setCustomVar(key, type, base, 0, preCalc, postCalc);
    }
    protected final void setCustomVar(String key, BaseSignatureCard.VariableType type, int base, int upgrade, TriFunction<BaseSignatureCard, AbstractMonster, Integer, Integer> preCalc, TriFunction<BaseSignatureCard, AbstractMonster, Integer, Integer> postCalc) {
        setCustomVarValue(key, base, upgrade);
    ...
    private void setCustomVarValue(String key, int base, int upg) {
        cardVariables.compute(key, (k, old)->{
            if (old == null) {
                return new BaseSignatureCard.LocalVarInfo(base, upg);
            }
            else {
                old.base = base;
                old.upgrade = upg;
    ...
    protected final void colorCustomVar(String key, Color normalColor, Color increasedColor, Color decreasedColor, Color upgradedColor) {
        BaseSignatureCard.LocalVarInfo var = getCustomVar(key);
        if (var == null) {
    ...
    private BaseSignatureCard.LocalVarInfo getCustomVar(String key) {
    ...
    protected void setVarCalculation(String key, TriFunction<BaseSignatureCard, AbstractMonster, Integer, Integer> calculation) {
        cardVariables.get(key).calculation = calculation;
    }

    public int customVarBase(String key) {
        BaseSignatureCard.LocalVarInfo var = cardVariables.get(key);
        if (var == null)
            return -1;
        return var.base;
    }
    public int customVar(String key) {
        BaseSignatureCard.LocalVarInfo var = cardVariables == null ? null : cardVariables.get(key); //Prevents crashing when used with dynamic text
        if (var == null)
            return -1;
        return var.value;
    }
    ...
    public AbstractCard makeStatEquivalentCopy() {
        ...
        if (candidate instanceof BaseSignatureCard) {
            BaseSignatureCard card = (BaseSignatureCard) candidate;
            ...
            for (Map.Entry<String, BaseSignatureCard.LocalVarInfo> varEntry : cardVariables.entrySet()) {
                BaseSignatureCard.LocalVarInfo target = card.getCustomVar(varEntry.getKey()),
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            for (BaseSignatureCard.LocalVarInfo var : cardVariables.values()) {
    ...
    protected void upgradeCustomVar(BaseSignatureCard.LocalVarInfo var) {
        upgradeCustomVar(var, var.upgrade);
    }

    protected void upgradeCustomVar(BaseSignatureCard.LocalVarInfo var, int amt) {
        if (amt != 0) {
            var.base += amt;
            var.value = var.base;
            var.upgraded = true;
    ...
    private static class QuickDynamicVariable extends DynamicVariable {
        ...
        private BaseSignatureCard current = null;
        ...
        public void setIsModified(AbstractCard c, boolean v) {
            if (c instanceof BaseSignatureCard) {
                BaseSignatureCard.LocalVarInfo var = ((BaseSignatureCard) c).getCustomVar(localKey);
                if (var != null)
                    var.forceModified = v;
            }
        }
        ...
        public boolean isModified(AbstractCard c) {
            return c instanceof BaseSignatureCard && (current = (BaseSignatureCard) c).isCustomVarModified(localKey);
        }

        @Override
        public int value(AbstractCard c) {
            return c instanceof BaseSignatureCard ? ((BaseSignatureCard) c).customVar(localKey) : 0;
        }

        @Override
        public int baseValue(AbstractCard c) {
            return c instanceof BaseSignatureCard ? ((BaseSignatureCard) c).customVarBase(localKey) : 0;
        }

        @Override
        public boolean upgraded(AbstractCard c) {
            return c instanceof BaseSignatureCard && ((BaseSignatureCard) c).customVarUpgraded(localKey);
        }

        public Color getNormalColor() {
            BaseSignatureCard.LocalVarInfo var;
    ...
    protected static class LocalVarInfo {
        ...
        TriFunction<BaseSignatureCard, AbstractMonster, Integer, Integer> calculation = BaseSignatureCard.LocalVarInfo::noCalc;
        ...
        private static int noCalc(BaseSignatureCard c, AbstractMonster m, int base) {
            return base;
        }
