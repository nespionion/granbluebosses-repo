...
public class JusticeTransferPower extends BaseCard {

    public static final String ID = makeID(JusticeTransferPower.class.getSimpleName());
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (p.hasPower(UnrighteousnessPower.POWER_ID) && m != null){
            addToBot(new DamageAction(p, new DamageInfo(p, this.damage, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.SLASH_DIAGONAL));
            addToBot(new RemoveSpecificPowerAction(p, p, UnrighteousnessPower.POWER_ID));
            addToBot(new ApplyPowerAction(m, p, new UnrighteousnessPower(m, true), 1));
        }
    }
