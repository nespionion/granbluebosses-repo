...
import granbluebosses.action.CancelOmenAction;
...
public class ArcarumJusticeCard extends BaseSignatureCard {

    public static final String ID = makeID(ArcarumJusticeCard.class.getSimpleName());
    ...
    private static final CardTags[] cardTags = {
            CustomTags.SUMMON_CALL, CustomTags.ARCARUM_CALL
    };
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        if (m.currentBlock % 2 == 0){
            addToBot(new GainBlockAction(p, m.currentBlock / 2));
            addToBot(new DamageAction(m, new DamageInfo(p, m.currentBlock / 2, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.SLASH_VERTICAL));
        } else {
            addToBot(new GainBlockAction(p, (m.currentBlock - 1) / 2));
            addToBot(new DamageAction(m, new DamageInfo(p, (m.currentBlock + 1) / 2, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.SLASH_VERTICAL));
        }
        ...
        for (AbstractMonster mo : AbstractDungeon.getMonsters().monsters){
            if (mo instanceof TheWorld && AbstractDungeon.ascensionLevel < 19){
                addToBot(new CancelOmenAction(m));
            }
        }
